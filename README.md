![LapLog — minimalist iOS stopwatch with renamable laps](LapLog_Banner.png)
...
Native SwiftUI. iOS 26.4+. Bundle ID `com.javierorraca.LapLog`.
...
- **Parallel laps mode** — flip on in Settings to let every lap keep ticking until you Stop. Useful when several things are cooking at once and you want to know how long each has been on.
...
- **Editable session title** — tap "SESSION · NAME" in the top-left to rename. Reverts to "New" when you start a new session.
...
- **Big numerals toggle** — bumps the timer readout up a size for visibility from across the room.
- **Export** — copy the current lap list to the clipboard as plain text.

## Design

Type is SF Pro Rounded with tabular numerals for the timer, monospaced caps for status strings, and a single accent color running through the controls and ring. A 60-second progress ring sits behind the readout to give a subtle sense of motion without distracting from the numbers.
